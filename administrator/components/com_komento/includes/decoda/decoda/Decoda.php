...
// Constants
if (!defined('KT_DECODA')) {
	define('KT_DECODA', dirname(__FILE__) . '/');
}

if (!defined('KT_DECODA_HOOKS')) {
	define('KT_DECODA_HOOKS', KT_DECODA . 'hooks/');
}

if (!defined('KT_DECODA_FILTERS')) {
	define('KT_DECODA_FILTERS', KT_DECODA . 'filters/');
}

if (!defined('KT_DECODA_EMOTICONS')) {
	define('KT_DECODA_EMOTICONS', KT_DECODA . 'emoticons/');
}

// Includes
include_once KT_DECODA . 'DecodaAbstract.php';
include_once KT_DECODA . 'DecodaHook.php';
include_once KT_DECODA . 'DecodaFilter.php';
include_once KT_DECODA . 'DecodaTemplateEngineInterface.php';
...
class KtDecoda {
	 ...
	 * @chainable
	 */
	public function addFilter(KtDecodaFilter $filter) {
	 ...
	 * @chainable
	 */
	public function addHook(KtDecodaHook $hook) {
		$hook->setParser($this);
	...
	public function loadFile($class) {
		...
		if (!JFile::exists(KT_DECODA_FILTERS . $class . '.php') && !JFile::exists(KT_DECODA_HOOKS . $class . '.php')) {
			return;
		}

		if (strpos($class, 'Filter') !== false) {
			include_once KT_DECODA_FILTERS . $class . '.php';

		} else if (strpos($class, 'Hook') !== false) {
			include_once KT_DECODA_HOOKS . $class . '.php';
		}
	}
	...
	public function reset($string, $flush = false) {
		...
		$this->addFilter(new KtEmptyFilter());
	 ...
	 * @chainable
	 */
	public function setTemplateEngine(KtDecodaTemplateEngineInterface $templateEngine) {
