...
Komento.ready(function($){
	if (!Komento.editorInitialized) {
		...
		Komento.bbcodeButtons = function() {
			...
			//[KOMENTO PAID START]
			<?php if ($this->config->get('bbcode_emoji')) { ?>
			settings.markupSet.push({
				name: "<?php echo JText::_('COM_KT_BBCODE_EMOJI', true); ?>",
			...
			//[KOMENTO PAID START]
			<?php if ($this->config->get('bbcode_giphy') && KT::giphy()->isEnabled()) { ?>
			settings.markupSet.push({
				name: "<?php echo JText::_('GIPHY', true); ?>",
				className: 'fd-markitup-giphy'
			});
			<?php } ?>
			//[KOMENTO PAID END]
			...
			settings.markupSet.push({
				...
				openWith:'[list=[![Starting number]!] type=decimal]\n[*]',
				closeWith:'\n[/list]',
...
.done(function($) {
	...
	$('[data-kt-structure="<?php echo $uniqid; ?>"]').implement(Komento.Controller.Structure, options);
});
<?php } ?>